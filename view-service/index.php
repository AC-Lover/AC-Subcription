...
<head>
    ...
    <script>
        ...
        const dataLimit = "<?php echo empty($user['data_limit']) ? '∞' : $user['data_limit']; ?>";
        const dataUsed = "<?php echo $user['used_traffic']; ?>";
        const expireDateInit = "<?php echo empty($user['expire']) ? '∞' : date('Y-m-d H:i:s', $user['expire']); ?>";
            ...
            <div class="flex flex-col sm:flex-row space-y-10 sm:space-y-0 sm:divide-x sm:rtl:divide-x-reverse sm:divide-blue-600/50">
                <div class="basis-1/3 space-y-4 flex flex-col items-center py-3 sm:ltr:pr-9 sm:rtl:pl-9">
                    ...
                    <span class="px-4 py-2 rounded-full inline-block shadow-md shadow-green-900 font-bold text-gray-200"
                          x-data="{status: '<?php echo $user['status']; ?>'}"
                          x-text="[status == 'active' ? $t('active') : status == 'limited' ? $t('limited') : status == 'expired' ? $t('expired') : $t('disabled')]"
                          :class="[status == 'active' ? 'bg-blue-600' : status == 'limited' ? 'bg-red-600' : status == 'expired' ? 'bg-orange-600' : 'bg-gray-600']"></span>
                    <span class="flex cursor-pointer" onclick="openSettings()">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                 class="w-6 h-6 stroke-blue-600 drop-shadow-lg" viewBox="0 0 24 24" stroke-width="1.5"
                                 stroke="currentColor">
                ...
                <div class="basis-2/3 flex flex-row items-center sm:ltr:pl-9 sm:rtl:pr-9">
                    <div class="data-usage w-full" x-data="progressBar"
                         x-init="Alpine.data( 'progressBar', () => progressBar )">
                        <div class="dark:text-white text-black text-center"><span class="font-bold"
                                                                                  x-text="$t('dataUsage')"></span><span
                                    dir="ltr"><?php echo bytesformat($user['used_traffic']); ?> / <?php echo empty($user['data_limit']) ? '∞' : bytesformat($user['data_limit']); ?></span>
                        </div>
                        <div class="bg-gray-200 dark:bg-gray-900 rounded-full h-6 mt-5 drop-shadow-lg"
                             role="progressbar" :aria-valuenow="width" aria-valuemin="0" aria-valuemax="100">
                            <div class="progress-bar rounded-full h-6 text-center dark:text-white text-black text-sm transition leading-6"
                                 :class="color" :style="`width: ${width}%; transition: width 2s;`"
                                 x-text="`${width}%`"></div>
                        </div>
                        <div class="dark:text-white text-black mt-10 text-center"><span class="font-bold"
                                                                                        x-text="$t('expirationDate')"></span><span
                                    dir="ltr" x-data="{expireDate: ''}"
                                    x-init="Alpine.data( 'expireDate', expireDate = expireDateVar )"
                                    x-text="expireDate"></span></div><!--2023/06/31 10:43:59-->
                        <div class="dark:text-white text-black mt-3 text-sm text-center"
                             x-text="resetInterval == 'year' ? $t('resetIntervalYear') : resetInterval == 'month' ? $t('resetIntervalMonth') : resetInterval == 'week' ? $t('resetIntervalWeek') : resetInterval == 'day' ? $t('resetIntervalDay') : ''"></div>
                        <div class="dark:text-white text-black mt-5 text-center"><span class="font-bold"
                                                                                       x-text="$t('remainingDays')"></span><span><?php echo empty($user['expire']) ? '∞' : '(' . intval(($user['expire'] - time()) / (24 * 3600)) . ')'; ?></span><span
                                    x-text="$t('remainingDaysSufix')"></span></div>
                    </div>
                </div>
                ...
                <div class="mb-4 border-b border-gray-200 dark:border-gray-700">
                    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="main-tab"
                        ...
                        <li class="flex-1" role="presentation">
                            <button class="inline-block p-4 border-b-2 rounded-t-lg w-full transition" id="profile-tab"
                                    data-tabs-target="#links" type="button" role="tab" aria-controls="links"
                                    aria-selected="false" x-text="$t('links')"></button>
                        </li>
                        <li class="flex-1" role="presentation">
                            <button class="inline-block w-full p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 transition"
                                    id="apps-tab" data-tabs-target="#apps" type="button" role="tab" aria-controls="apps"
                                    aria-selected="false" x-text="$t('apps')"></button>
                        </li>
                        <li class="flex-1" role="presentation">
                            <button class="inline-block w-full p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 transition"
                                    id="tutorials-tab" data-tabs-target="#tutorials" type="button" role="tab"
                                    aria-controls="tutorials" aria-selected="false" x-text="$t('tutorials')"></button>
                        </li>
                    </ul>
                </div>
                <div id="tabs-content">
                    <div class="hidden" id="links" role="tabpanel" aria-labelledby="links-tab">
                        <ul class="list-none p-0 m-0">
                            <li class="flex px-3 mb-3 justify-between leading-[3.5rem] bg-black/20 rounded-md shadow-lg"
                                x-data>
                                <span class="font-semibold flex-1 dark:text-gray-200 text-black cursor-default"
                                      x-text="$t('subscription')"></span>
                                <div class="flex justify-between items-center">
                                    <div class="w-8 h-8 ltr:mr-3 rtl:ml-3 cursor-pointer"
                                         x-data="{copyColor: 'stroke-blue-600'}"
                                         @click="() => { navigator.clipboard.writeText( '<?php echo $user['subscription_url']; ?>' ); copyColor = 'stroke-green-600'; setTimeout(() => copyColor = 'stroke-blue-600', 2000); }">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                             class="dark:hover:stroke-gray-300 hover:stroke-gray-800 transition-colors"
                                             :class="copyColor" viewBox="0 0 24 24" stroke-width="1.5"
                                             stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                  d="M15.666 3.888A2.25 2.25 0 0013.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 01-.75.75H9a.75.75 0 01-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 01-2.25 2.25H6.75A2.25 2.25 0 014.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 011.927-.184"/>
                                        </svg>
                                    </div>
                                    <div class="w-8 h-8 cursor-pointer qr-button-single"
                                         data-qrlink="<?php echo $user['subscription_url']; ?>">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                            ...
                            <?php foreach ($user['links'] as $link): ?>
                                <li class="flex px-3 mb-1 justify-between leading-[3.5rem] hover:bg-black/10 rounded-md hover:shadow-lg transition duration-300"
                                    x-data="{link: '<?php echo $link; ?>'}">
                                    <span class="font-semibold flex-1 dark:text-gray-200 text-black cursor-default"
                                          x-text="getRemark(link)"></span>
                                    <div class="flex justify-between items-center">
                                        <div class="w-8 h-8 ltr:mr-3 rtl:ml-3 cursor-pointer"
                                             x-data="{copyColor: 'stroke-blue-600'}"
                            ...
                            <li class="rounded-md shadow-lg transition duration-300 bg-blue-600 text-white text-center text-lg py-2 mt-3 cursor-pointer hover:shadow-2xl copyAllButton"
                                x-text="$t('copyAll')"></li>
                        </ul>
                    </div>
                                    ...
                                    <template x-for="item in Object.getOwnPropertyNames(appsJson)">
                                        <li class="flex-grow mb-2" role="presentation">
                                            <button class="inline-block p-4 border-b-2 rounded-t-lg w-full transition"
                                                    ...
                                                    role="tab" :aria-controls="item" aria-selected="false"
                                                    x-text="item"></button>
                                        </li>
                                    </template>
                                </ul>
                                            ...
                                            <template x-for="app in Object.getOwnPropertyNames(appsJson[item])">
                                                <template x-for="subApp in appsJson[item][app].url">
                                                    <li class="flex px-3 mb-1 justify-between leading-[3.5rem] hover:bg-black/10 rounded-md hover:shadow-lg transition duration-300"
                                                        ...
                                                        <div class="flex justify-between items-center">
                                                            <a class="w-8 h-8 cursor-pointer" :href="link"
                                                               target="_blank">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                                     class="stroke-blue-600 dark:hover:stroke-gray-300 hover:stroke-gray-800 transition-colors"
                                    ...
                                    <template x-for="item in Object.getOwnPropertyNames(appsJson)">
                                        <li class="flex-grow mb-2" role="presentation">
                                            <button class="inline-block p-4 border-b-2 rounded-t-lg w-full transition"
                                                    ...
                                                    :data-tabs-target="'#' + item + '-tutorials'" type="button"
                                                    role="tab" :aria-controls="item + '-tutorials'"
                                                    aria-selected="false" x-text="item"></button>
                                        </li>
                                    </template>
                                </ul>
                            </div>
                                            ...
                                            <template x-for="app in Object.getOwnPropertyNames(appsJson[item])">
                                                <li class="flex px-3 mb-1 justify-between leading-[3.5rem] hover:bg-black/10 rounded-md hover:shadow-lg transition duration-300">
                                                    <span class="font-semibold flex-1 dark:text-gray-200 text-black cursor-default"
                                                          x-text="app"></span>
                                                    <div class="flex justify-between items-center">
                                                        <button class="w-8 h-8 cursor-pointer video-button"
...
<script>
    ...
    let qrButtonSingle = $(".qr-button-single");
    qrButtonSingle.on('click', () => {
